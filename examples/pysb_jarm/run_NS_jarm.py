...
from gleipnir.samplers import MetropolisSampler
...
def loglikelihood(position):
    ...
    # #    boxes = evaluate_cycles(pars1)
    # # print(boxes)
    # logp_boxes = like_thermobox.logpdf(boxes)
    #quit()
...
if __name__ == '__main__':
    ...
    sampler = MetropolisSampler(iterations=10, burn_in=20)
    # Setup the stopping criterion for the NS run -- We'll use a fixed number of
    # iterations: 10*population_size
    stopping_criterion = NumberOfIterations(100*population_size)
    ...
    print("prior to posterior compression: ", np.exp(-information))
    print("log10 prior to posterior compression: ", np.log10(np.exp(-information)))
    ...
    try:
        ...
        for parm in posteriors.keys():
            ...
            plt.savefig("f_post_marginal_dist_parm_{}.pdf".format(parm))
            plt.close()
